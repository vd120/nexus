    ...
    return new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = (e) => {
            ...
            if (!database.objectStoreNames.contains(STORE_QUEUE)) {
                ...
                store.createIndex("status_seq", ["status", "sequenceNumber"], {
                    unique: false,
                });
...
async function replayItem(item) {
    ...
    try {
        ...
        if (item.type === "post_reaction") {
            ...
            const url =
                (item.data && item.data.store_url) || `/chat/${item.recordId}`;
...
export function init() {
    ...
    openDB()
        .then(async () => {
            const items = await getPendingItems();
            sequenceCounter = items.reduce(
                (max, i) => Math.max(max, i.sequenceNumber),
                0,
            );
        })
        .catch(() => {});
