...
(function () {
    ...
    function vib(pattern) {
        ...
        if (reducedMotion) {
            pattern = (Array.isArray(pattern) ? pattern : [pattern]).map((v) =>
                Math.min(v, 12),
            );
        }
        try {
    ...
    const Sounds = {
        ...
        success: () => {
            ...
            setTimeout(
                () => tone({ freq: 659, dur: 0.09, type: "sine", peak: 0.38 }),
                85,
            );
            setTimeout(
                () => tone({ freq: 784, dur: 0.13, type: "sine", peak: 0.42 }),
                170,
            );
        },
        // Delete — low thud
