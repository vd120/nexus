(function () {
    ...
    function detectToken(textarea) {
        ...
        while (i >= 0) {
            ...
            if (ch === '@' || ch === '#') {
                ...
                if (query.length && !/^[\p{L}\p{N}_-]+$/u.test(query)) return null;
    ...
    function renderUsers(users) {
        ...
        popoverItems = users.map(function (u) { return { type: 'user', value: u.username, data: u }; });
        popover.innerHTML = users.map(function (u, idx) {
            const initial = String(u.username || u.name || '?').slice(0, 1).toUpperCase();
    ...
    function renderHashtags(tags) {
        ...
        popoverItems = tags.map(function (t) { return { type: 'hashtag', value: t.name, data: t }; });
        popover.innerHTML = tags.map(function (t, idx) {
            const count = t.usage_count != null
                ? '<span class="mh-secondary">' + t.usage_count + ' posts</span>'
                : '';
            return '<div class="mh-item' + (idx === activeIndex ? ' mh-active' : '') + '" role="option" data-mh-index="' + idx + '">'
                + '<div class="mh-hashicon">#</div>'
                + '<div class="mh-meta">'
                +   '<span class="mh-primary">#' + escapeHtml(t.name || '') + '</span>'
                +   count
                + '</div>'
                + '</div>';
        }).join('');
    ...
    async function fetchSuggestions(trigger, query) {
        ...
        try {
            if (trigger === '@') {
                const url = '/api/users/following/suggestions?search=' + encodeURIComponent(query || '');
            ...
            // hashtag
            if (!query) {
                const r = await fetch('/api/sidebar/trending-hashtags?limit=' + MAX_RESULTS, opts);
                if (!r.ok) return [];
                const data = await r.json();
                return Array.isArray(data.data) ? data.data : [];
            }
            const r = await fetch('/api/hashtags/suggestions?search=' + encodeURIComponent(query), opts);
            if (!r.ok) return [];
            const data = await r.json();
    ...
    function handleInput(e) {
        ...
        if (queryKey === lastQueryKey && popover && popover.style.display !== 'none') return;
    ...
    function selectItem(idx) {
        if (!popoverItems.length || !activeTextarea || activeTokenStart < 0) return;
        const item = popoverItems[idx];
        if (!item) return;
        const replacement = (item.type === 'user' ? '@' : '#') + item.value + ' ';
    ...
    function handleKeydown(e) {
        if (!popover || popover.style.display === 'none' || !popoverItems.length) return;
        ...
        if (e.key === 'ArrowDown') {
            ...
            e.preventDefault();
            activeIndex = (activeIndex - 1 + popoverItems.length) % popoverItems.length;
            updateActiveStyles();
    ...
    function attach() {
        ...
        const reposition = function () {
            if (popover && popover.style.display !== 'none' && activeTextarea) positionPopover(activeTextarea);
        };
