(function () {
    ...
    function initSocket() {
        ...
        if (socket) {
            console.log(
                "%c [Chat] Sending join: global-chat ",
                "color: #ffc107;",
            );
            ...
            socket.on("chat:message", (data) => {
                if (
                    data.conversation_id === "global-chat" &&
                    window.addMessage
                ) {
            ...
            socket.on("chat:reaction", (data) => {
                if (
                    data.conversation_id === "global-chat" &&
                    window.updateReactionsFromSocket
                ) {
            ...
            socket.on("chat:message:deleted", (data) => {
                if (data.conversation_id === "global-chat") {
                    const msgEl = document.querySelector(
                        `.message-item[data-id="${data.message_id}"]`,
                    );
                    if (msgEl) {
                        const bubble = msgEl.querySelector(".message-bubble");
                        if (bubble) {
                            const deletedText =
                                chatMessages.getAttribute(
                                    "data-message-deleted-text",
                        ...
                        const reactBtn =
                            msgEl.querySelector(".quick-react-btn");
    ...
    window.movePreview = function (dir) {
        ...
        if (currentPreviewIndex < 0)
            currentPreviewIndex = selectedMediaFiles.length - 1;
    ...
    window.addMessage = function (data) {
        ...
        const senderUsername = data.sender
            ...
            : data.username || "user";
        const senderAvatar = data.sender
        ...
        const hasReactions =
            data.reactions && Array.from(data.reactions).length > 0;
        ...
        const time =
            String(_th).padStart(2, "0") +
            ":" +
            _tm +
            " " +
            (_td.getHours() >= 12 ? "pm" : "am");
        ...
        if (isAtBottom) {
            ...
            unreadScrollBadge.textContent =
                unreadCountWhileScrolled > 99
                    ? "99+"
                    : unreadCountWhileScrolled;
    ...
    window.updateReactionsFromSocket = function (data) {
        ...
        if (data.reactions && data.reactions.length > 0) {
            const totalCount = data.reactions.reduce(
                (sum, r) => sum + r.count,
                0,
            );
    ...
    window.toggleMsgMenu = function (event, id) {
        ...
        if (!isShowing) {
            if (window.innerWidth <= 768) {
                ...
                const btn =
                    event.currentTarget ||
                    event.target.closest(".msg-action-trigger");
    ...
    window.closeAllMenus = function () {
        document
            .querySelectorAll(".msg-dropdown")
            .forEach((d) => d.classList.remove("show"));
        document
            .querySelectorAll(".message-item")
            .forEach((i) => (i.style.zIndex = ""));
        const backdrop = document.getElementById("msgOptsBackdrop");
    ...
    window.confirmClearChat = async function () {
        if (
            !confirm(
                "Are you sure you want to CLEAR the entire global chat? This cannot be undone.",
            )
        )
            return;
    ...
    document.addEventListener("click", (e) => {
        if (
            ...
            !e.target.closest(".msg-action-trigger")
        ) {
            window.closeAllMenus();
        }
    });
        ...
        fetch(`/global-chat/message/${messageId}/reactions`)
            ...
            .then((data) => {
                if (
                    ...
                    list.innerHTML =
                        '<div style="padding: 20px; text-align: center; color: #8696a0;">No reactions found.</div>';
                    return;
                }
                ...
                data.reactions.forEach((group) => {
                    group.users.forEach((user) => {
                        const isMe =
                            String(user.id) ===
                            String(window.SOCKET_CONFIG.userId);
                        const avatar =
                            user.avatar_url || "/images/default-avatar.svg";
                        const emoji = group.reaction_type;

                        const gcBadge = user.is_verified
                            ? `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width=".8em" height=".8em" style="display:inline-block;vertical-align:middle;margin-left:.15em;flex-shrink:0;" aria-label="Verified" role="img"><circle cx="12" cy="12" r="10.5" fill="#1d9bf0"/><path d="M7 12.5l3 3 7-7" stroke="#fff" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>`
                            : "";
                        html += `
            ...
            .catch((err) => {
                console.error(err);
                list.innerHTML =
                    '<div style="padding: 20px; text-align: center; color: #f44336;">Error loading reactions.</div>';
            });
    };
    ...
    function updateGalleryContent() {
        ...
        prevBtn.style.visibility =
            currentGalleryIndex > 0 ? "visible" : "hidden";
        nextBtn.style.visibility =
            currentGalleryIndex < currentGalleryMedia.length - 1
                ? "visible"
                : "hidden";
    }
