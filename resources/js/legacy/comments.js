    ...
    window.handleTargetedComment = function() {
        ...
        if (hash && hash.startsWith('#comment-')) {
            ...
            const findAndShow = () => {
                ...
                if (commentEl) {
                    console.log('[Comments] Found targeted comment:', commentId);
                    ...
                    while (parent && parent !== document.body) {
                        ...
                        if (parent.classList.contains('hidden-comments')) {
                            const postId = parent.id.replace('hidden-comments-', '');
                            console.log('[Comments] Revealing hidden comments for post:', postId);
                        ...
                        if (parent.classList.contains('hidden-replies')) {
                            const parentCommentId = parent.id.replace('hidden-replies-', '');
                            console.log('[Comments] Revealing hidden replies for comment:', parentCommentId);
                            if (window.toggleNestedReplies) {
                                window.toggleNestedReplies(parentCommentId, true);
                            } else {
                                parent.style.display = 'block';
                            }
                        }
