...
(function () {
        ...
        const safePlay = () => {
            ...
            if (playPromise !== undefined) {
                playPromise.catch((error) => {
                    ...
                    console.debug(
                        "Video play interrupted or prevented:",
                        error,
                    );
                });
            }
        ...
        function updateThemeIcons(theme) {
            document.querySelectorAll(".theme-option-btn").forEach((btn) => {
                btn.classList.toggle(
                    "active",
                    btn.getAttribute("data-theme-btn") === theme,
                );
            });
    ...
    // GSAP Animations
    window.initGSAP = function (immediate) {
        if (typeof gsap === "undefined") return;

        gsap.registerPlugin(ScrollTrigger);

        // Hero entrance is now handled by CSS fades via .content-loaded class
        // initGSAP remains for scroll-triggered animations in other sections

        // Section 1: "Built for Real Connections" - Fade
        const fadeSection = document.querySelector(
            "#section-fade .fade-content",
        );
        if (fadeSection) {
            gsap.timeline({
                scrollTrigger: {
        ...
        // Final refresh to ensure all triggers are calculated correctly
        setTimeout(() => {
    ...
    // Initialize GSAP when DOM is ready - but only if NOT on landing page with intro
    window.runOnPageLoad(function () {
        if (typeof gsap !== "undefined") {
            const intro = document.getElementById("intro-wrapper-id");
            // If intro exists, home.blade.php handles the call to initGSAP
            if (!intro) {
                window.initGSAP();
            }
        }
