...
(function () {
    ...
    const sessionMessages = {
        ...
        ar: {
            "passwords.reset":
                "كلمة السر اتغيرت بنجاح! يمكنك تسجيل الدخول الآن.",
    ...
    function getSessionMessage(key, lang) {
        if (key && sessionMessages[lang]?.[key])
            return sessionMessages[lang][key];
        ...
        if (
            keyLower.includes("كلمة") ||
            keyLower.includes("كلمة السر") ||
            keyLower.includes("اتغيرت")
        ) {
            return (
                sessionMessages[lang]?.["passwords.reset"] ||
                sessionMessages.en["passwords.reset"]
            );
        }
        ...
        if (keyLower.includes("suspended")) {
            return (
                sessionMessages[lang]?.["account_suspended"] ||
                sessionMessages.en["account_suspended"]
            );
        }
        if (keyLower.includes("concurrent")) {
            return (
                sessionMessages[lang]?.["concurrent_login"] ||
                sessionMessages.en["concurrent_login"]
            );
        }
        if (keyLower.includes("deleted")) {
            return (
                sessionMessages[lang]?.["account_deleted"] ||
                sessionMessages.en["account_deleted"]
            );
        }
        if (keyLower.includes("logged out") || keyLower.includes("logout")) {
            return (
                sessionMessages[lang]?.["logged_out"] ||
                sessionMessages.en["logged_out"]
            );
        }
    ...
    window.runOnPageLoad(function () {
        ...
        if (configEl) {
            ...
            if (concurrent) {
                const concurrentMsg = getSessionMessage(
                    "concurrent_login",
                    userLang,
                );
            ...
            if (deleted) {
                const deletedMsg = getSessionMessage(
                    "account_deleted",
                    userLang,
                );
            ...
            if (suspended) {
                const suspendedMsg = getSessionMessage(
                    "account_suspended",
                    userLang,
                );
            ...
            if (urlError)
                showSessionToast(
                    "error",
                    getSessionMessage(urlError, userLang),
                );
            if (urlMessage)
                showSessionToast(
                    "success",
                    getSessionMessage(urlMessage, userLang),
                );
        }
    });
})();
