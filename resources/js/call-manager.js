...
(function () {
    ...
    const ICE_SERVERS = (function () {
        ...
        var extra =
            window.CALL_CONFIG && Array.isArray(window.CALL_CONFIG.iceServers)
    ...
    function _ensureRingBuffer() {
        ...
        if (ctx && !_ringBuffer && ctx.state === "running") {
            try {
                _ringBuffer = buildRingBuffer(ctx);
            } catch (e) {}
        }
    }
    ...
    function getRingSrc() {
        ...
        var SR = 22050,
            ringLen = Math.ceil(SR * 1.0),
            totalLen = Math.ceil(SR * 3.0);
        var ab = new ArrayBuffer(44 + totalLen * 2),
            v = new DataView(ab);
        function ws(o, s) {
    ...
    function tryUnlockElement(el) {
        ...
        el.play()
            .then(function () {
                ...
                if (
                    el === document.getElementById("call-remote-audio") &&
                    el.srcObject
                )
                    return;
                el.pause();
    ...
    function startTimer() {
        ...
        timerInterval = setInterval(() => {
            ...
            if (
                window.CallModal &&
                typeof window.CallModal.updateTimer === "function"
            ) {
    ...
    function createPeerConnection(socket) {
        ...
        pc.oniceconnectionstatechange = () => {
            ...
            if (state === "failed") {
                ...
                if (pc._iceFailedTimer) {
                    clearTimeout(pc._iceFailedTimer);
                    pc._iceFailedTimer = null;
                }
    ...
    async function flushIceCandidateBuffer() {
        ...
        for (const candidate of toFlush) {
            try {
                ...
                console.warn(
                    "[CallManager] Failed to add buffered ICE candidate:",
                    err,
                );
            }
        }
    ...
    function startCall(
        ...
        if (
            window.CallModal &&
            typeof window.CallModal.showCalling === "function"
        ) {
        ...
        noAnswerTimeout = setTimeout(function () {
            ...
            resetState();
            if (window.CallModal)
                window.CallModal.showStatus(t.call_no_answer || "No answer");
    ...
    function attachSocketListeners(socket) {
        ...
        socket.on("call:incoming", (data) => {
            ...
            if (
                window.CallModal &&
                typeof window.CallModal.showIncoming === "function"
            ) {
        ...
        socket.on("call:accepted", async (data) => {
            ...
            try {
                ...
                if (
                    window.CallModal &&
                    typeof window.CallModal.showActive === "function"
                ) {
        ...
        socket.on("call:callee-ready", (data) => {
            ...
            if (data.callId && data.callId !== peerConnection._pendingOfferCall)
                return;
            socket.emit("call:offer", {
        ...
        socket.on("call:rejected", (data) => {
            ...
            if (
                window.CallModal &&
                typeof window.CallModal.showStatus === "function"
            ) {
                window.CallModal.showStatus(
                    (window.CallTranslations || {}).call_declined ||
                        "Call declined",
                );
            }
        ...
        socket.on("call:answer", async (data) => {
            ...
            try {
                ...
                console.error(
                    "[CallManager] Error setting remote description:",
                    err,
                );
                endCall(socket);
            }
        ...
        socket.on("call:no-answer", () => {
            ...
            if (
                ...
                window.CallModal.showStatus(
                    (window.CallTranslations || {}).call_no_answer ||
                        "No answer",
                );
            }
        });
    ...
    window.CallManager = {
        ...
        init(socket) {
            if (!socket) {
                console.error(
                    "[CallManager] init() called without a socket instance.",
                );
                return;
            }
        ...
        acceptCall() {
            ...
            fetch(`/call/${callId}/accept`, {
                ...
                console.warn("[CallManager] accept POST failed:", err),
            );
        ...
        rejectCall() {
            ...
            fetch(`/call/${callId}/reject`, {
                ...
                console.warn("[CallManager] reject POST failed:", err),
            );
            ...
            if (
                window.CallModal &&
                typeof window.CallModal.hide === "function"
            ) {
                window.CallModal.hide();
            }
        },
        ...
        toggleMute() {
            ...
            if (
                window.CallModal &&
                typeof window.CallModal.setMuted === "function"
            ) {
        ...
        async toggleSpeaker() {
                ...
                if (typeof remoteAudio.setSinkId === "function") {
                    try {
                        ...
                        const target = speakerEnabled
                            ? speakers.find((d) => d.deviceId === "default")
                                  ?.deviceId || "default"
                            : speakers.find((d) => d.deviceId !== "default")
                                  ?.deviceId || "";
            ...
            if (
                window.CallModal &&
                typeof window.CallModal.setSpeaker === "function"
            ) {
